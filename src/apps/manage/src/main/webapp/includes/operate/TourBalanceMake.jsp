...
<meta name="heading" content="输入单团核算单">
</head>
...
function DeleteRow(param)
{
   ...
   fm.action = "<s:url action='TourBalanceMakeDelete' namespace='/operator'/>";
   fm.submit();
}
...
function change(para,param2)
{
   ...
   var roe=document.getElementById('TourBalanceMake_'+ para + '_roe');
   var unitPrice=document.getElementById('TourBalanceMake_'+ para + '_unitPrice');
   var count=document.getElementById('TourBalanceMake_'+ para + '_count');
   var amount=document.getElementById('TourBalanceMake_'+ para + '_amount');
   var cost=document.getElementById("TourBalanceMake_singleTourBalance_cost");
   var grossAmount=document.getElementById("TourBalanceMake_singleTourBalance_grossAmount");
   var grossAmountRate=document.getElementById("TourBalanceMake_singleTourBalance_grossAmountRate");
   var tourAmount=document.getElementById("TourBalanceMake_singleTourBalance_tourAmount");
...
function paxChange(param)
{
  ...
  var allPax=document.getElementById("TourBalanceMake_singleTourBalance_allPax");
...
function checkOut(para,para1)
{
  var fm=document.TourBalanceMake;
  var extrIncome=document.getElementById("TourBalanceMake_singleTourBalance_extrIncome");
  var extrIncomeDec=document.getElementById("TourBalanceMake_singleTourBalance_extrIncomeDec");
  ...
  {
     supplierId=document.getElementById('TourBalanceMake_costList('+ i + ')_customer_customerId');
     unitPrice=document.getElementById('TourBalanceMake_costList('+ i + ')_unitPrice');
     count=document.getElementById('TourBalanceMake_costList('+ i + ')_count');
  ...
  var num=cost.value-tourAmount.value;
   ...
   fm.action = "<s:url action='TourBalanceMake' namespace='/operator'/>";
   fm.submit();
}
  ...
  function reCount(param)
  {
    ...
    var amount;
	  ...
	  {
	     roe=document.getElementById('TourBalanceMake_costList('+ i + ')_roe');
	     unitPrice=document.getElementById('TourBalanceMake_costList('+ i + ')_unitPrice');
       count=document.getElementById('TourBalanceMake_costList('+ i + ')_count');
	     amount=document.getElementById('TourBalanceMake_costList('+ i + ')_amount');
  ...
  function extrIncomeChange(param)
  {
    ...
    var cost=document.getElementById("TourBalanceMake_singleTourBalance_cost");
	  ...
	  var grossAmountRate=document.getElementById("TourBalanceMake_singleTourBalance_grossAmountRate");
    var tourAmount=document.getElementById("TourBalanceMake_singleTourBalance_tourAmount");
    var allAmount=document.getElementById("TourBalanceMake_singleTourBalance_amount");
    var muAmount=document.getElementById("TourBalanceMake_singleTourBalance_muAmount");
...
<s:if test='status!="Y"'>
  <s:form action="TourBalanceMake" namespace="/operator" method="post" theme="simple">
    ...
    <table border="0" style="width: 100%">
              ...
              <tr>
                ...
                <td class="cdata">
                 <s:select cssStyle="background-color: EEEEEE;"
                           ...
                           value="%{supplierId}" 
                           disabled="true"/>
                  <s:hidden name="costList(%{id}).supplierId" value="%{supplierId}"></s:hidden>
                 </td>
                <td class="cdata">
                <s:textfield cssStyle="background-color: EEEEEE;" name="costList(%{id}).description" value="%{description}"  readonly="true"/>
                </td>
                <td class="cdata">
                  <s:textfield cssStyle="background-color: EEEEEE;" name="costList(%{id}).currency" value="%{currency}"  readonly="true" size="8"/>
                </td>
                <td class="cdata"><s:textfield cssStyle="background-color: EEEEEE;" name="costList(%{id}).roe" value="%{roe}"  readonly="true"/></td>
                <td class="cdata"><s:textfield cssStyle="background-color: EEEEEE;" name="costList(%{id}).unitPrice" readonly="true" value="%{unitPrice}" size="10"
                  onchange="javascript:change('costList(%{id})',this.id)" /></td>
                <td class="cdata"><s:textfield cssStyle="background-color: EEEEEE;" name="costList(%{id}).count" readonly="true" value="%{count}" size="3"
                  onchange="javascript:countchange('costList(%{id})',this.id)" /></td>
                <td class="cdata"><s:textfield cssStyle="background-color: EEEEEE;" name="costList(%{id}).unit"  readonly="true" value="%{unit}" size="6" /></td>
                <td class="cdata"><s:textfield cssStyle="background-color: EEEEEE;" name="costList(%{id}).amount"  readonly="true" value="%{amount}" size="10" /></td>
                <td class="cdata"><font color="red">财务已审核</font>
              ...
              <tr>
                <td class="cdata">
                  ...
                  <s:textfield name="costList(%{id}).id"
                               ...
                               readonly="true"/></td>
                <td class="cdata">
                <sj:autocompleter id="costList(%{id}).costType"
                                  name="costList(%{id}).costType"
			                            list="costTypeList"
			                            listKey="label"
			                            listValue="label"
			                            autoComplete="true" 
                ...
                <s:else>
                 <s:select name="costList(%{id}).customer.customerId"
                          list="supplierList"
                          listKey="supplierId"
                          listValue="supplierName"
                          headerKey=""
                ...
                <td class="cdata"><s:textfield name="costList(%{id}).description" value="%{description}" /></td>
                <!-- 币种 -->
                <td class="cdata">
                  <s:select name="costList(%{id}).currency"
                          ...
                          listKey="label"
                          listValue="value"
                          value="%{currency}"/><font color="red">*</font>
                </td>
                <td class="cdata"><s:textfield name="costList(%{id}).roe" value="%{roe}" onchange="javascript:change('costList(%{id})',this.id)"/></td>
                <td class="cdata"><s:textfield name="costList(%{id}).unitPrice" value="%{unitPrice}" size="10"
                  onchange="javascript:change('costList(%{id})',this.id)" /></td>
                <td class="cdata"><s:textfield name="costList(%{id}).count" value="%{count}" size="3"
                  onchange="javascript:countchange('costList(%{id})',this.id)" /></td>
      ...
      <tr>
        <td colspan="6" align="center">
          <s:submit action="TourBalanceMakeAdd" value="增加一行"></s:submit> 
          <input type="button" value="保存" onClick="javascript:checkOut('<s:property value="costList.size()" />','save')">
          <input type="button" value="保存并提交财务" onClick="javascript:checkOut('<s:property value="costList.size()" />','audit')">
        </td>
      </tr>
      <tr>
        <td colspan="6">&nbsp;</td>
