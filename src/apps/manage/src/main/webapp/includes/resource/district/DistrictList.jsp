...
<body>
<s:form action="searchDistrict" namespace="/resource" method="POST" theme="simple">
  <s:hidden name="districtNo"></s:hidden>
  <table style="width: 100%">
    <tr>
      <td class="idx" style="width: 80px;">名称:</td>
    ...
    <tr>
      ...
      <td>
        <s:select id="kenCountry"
                  ...
                  headerKey="" onchange="countryChange();"
                  headerValue="全部">
          </s:select> <span id="province_box"> 所在省
...
function SubmitForm(param, target)
{
  ...
  if (param =='delete')
  {
    ...
    fm.action = "<s:url action='deleteDistrict' namespace='/resource'/>"
  }
  if(param=='modify')
  {
    fm.action = "<s:url action='editDistrict' namespace='/resource'/>"
  }
  fm.submit();
}
