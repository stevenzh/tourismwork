...
{
  var fm = document.searchList;
  fm.listId.value=target;
  if (param =='delete')
  {
  ...
  if(param=='modify')
    fm.action = "<s:url action='editList' namespace='/setting'/>"
  fm.submit();
}
...
<s:form action="searchList" namespace="/setting" method="POST" theme="simple">
  <s:hidden name="listId"></s:hidden>
  <table style="width: 100%">
    <tr>
      <td colspan="6" align="right">
      <s:submit action="editList" value="添加新的列表"></s:submit>
    ...
    <s:iterator value="tmplList" status="rowcounter">
      ...
      <s:if test="#rowcounter.count <= toRecord">
        <tr>
          ...
          <td class="cdata"><s:property value="listName" />&nbsp;</td>
          <td class="cdata"><s:property value="text" />&nbsp;</td>
          <td class="data"><s:property value="value" />&nbsp;</td>
          <td class="cdata"><s:property value="sortOrder" />&nbsp;</td>
          <td class="cdata"><a href="#" onClick="javascript:SubmitForm('modify','<s:property value="entryId" />')">修改</a>&nbsp;&nbsp;
          <a href="#" onClick="javascript:SubmitForm('delete','<s:property value="entryId" />')">取消</a></td>
        </tr>
      </s:if>
      </s:if>
    </s:iterator>
