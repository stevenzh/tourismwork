    ...
    public void before(Method method, Object[] args, Object target) throws Throwable {
        ...
        if (ctx.getAuthentication() != null) {
            ...
            if (!signupUser) {
                ...
                if (user.getId() != null && !user.getId().equals(currentUser.getUsername()) && !administrator) {
                    log.warn("Access Denied: '" + currentUser.getUsername() + "' tried to modify '" + user.getUsername() + "'!");
                    throw new AccessDeniedException(ACCESS_DENIED);
                } else if (user.getId() != null && user.getId().equals(currentUser.getUsername()) && !administrator) {
                    // get the list of roles the user is trying add
                    Set<String> userRoles = new HashSet<String>();
                    if (user.getRoles() != null) {
                        for (Object o : user.getRoles()) {
                            PortalRole role = (PortalRole) o;
    ...
    throws Throwable {
        ...
        if (user.getVersion() != null) {
            ...
            if (auth != null && !signupUser) {
                User currentUser = getCurrentUser(auth);
                if (currentUser.getUsername().equals(user.getId())) {
